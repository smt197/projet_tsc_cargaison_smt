...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($input['action']) && $input['action'] === 'ouvrirCargaison') {
        $idcargo = $input['id'];
        ...
        if ($data['cargaisons'][$cargaisonKey]['etat_avancement'] === 'en_route') {
            echo json_encode(['status' => 'error', 'message' => 'Cargaison déjà en route, on ne peut plus l\'ouvrir']);
            exit;
        }
        ...
        echo json_encode(['status' => 'success', 'message' => 'Cargaison ouverte avec succès']);
        exit;
    }
